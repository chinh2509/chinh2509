
package giftview;
import giftcontroller.GiftController;
import giftmodel.Gift;
import java.util.Scanner;

public class GiftShop {
    final int SENTINEL = 0;
    int n;
    Scanner scanner = new Scanner(System.in);
    GiftController controller = new GiftController();
    public static void main(String[] args) {
        GiftShop gs = new GiftShop();
        gs.ViewShop();
    }
    public void menu() {
        System.out.println("==========================================================================");
        System.out.println("1 - Hiển thị toàn bộ danh sách có trong thư việc");
        System.out.println("2 - Nhập thông tin sách và insert");
        System.out.println("3 - Nhập id sách muốn xóa và delete ");
    }
    public void insertBook(){
        System.out.println("Insert into id: ");
        int id = scanner.nextInt();
        System.out.println("Insert into  name: ");
        String name = scanner.nextLine();
        name = scanner.nextLine();
        System.out.println("Insert into  price: ");
        double price = scanner.nextDouble();
        System.out.println("Insert into  qty: ");
        int qty = scanner.nextInt();
        Gift gift = new Gift(id, name, price, qty);
        controller.InsertGift(gift);
    }
    public void deleteBook(){
        System.out.println("Insert into id: ");
        int id = scanner.nextInt();
        controller.DeleteGift(id);
    }



    public void ViewShop() {
        System.out.println("hello welcome for GiftShop");
        GiftShop gs = new GiftShop();
        gs.menu();
        n = scanner.nextInt();
    while (n != SENTINEL ){
        switch (n){
            case 1:
                System.out.println("book list:");
                controller.selectBook();
                break;
              case 2:
                  System.out.println("insert into new book:");
                  gs.insertBook();
                  break;
            case 3:
                System.out.println("insert book id want delete");
                gs.deleteBook();
                break;
        }
        gs.menu();
        n = scanner.nextInt();
    }
        System.out.println("kết thúc chương trình<3<3<3");
    }
}
